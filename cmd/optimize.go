...
var mapToOptimize, mapGoodCount, mapRandomCount, mapAdjustCount int
var optimizeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fmt.Printf("Running optimization of map %s with %d cycles, %d random map per cycle and %d adjustments for each random map\n\n", mapMaskName, mapGoodCount, mapRandomCount, mapAdjustCount)
		optimize(mapMaskName)
		return nil
	},
}

func optimize(mapMaskName string) {
	...
	bestMap.Print()
	fmt.Printf("\nFinal map for %s scored: %.2f\n", mapMaskName, bestMap.Score())
}

func findBestMap(mask maps.MapMask) *maps.Map {
...
func findGoodMap(mask maps.MapMask) *maps.Map {
